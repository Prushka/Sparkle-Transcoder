...
```bash
/Users/dan/sdk/go1.26.3/bin/go run ./cmd/server
```

Default paths:

- `MEDIA_ROOT=/Volumes/media/Managed-Videos`
- `OUTPUT=/Volumes/media/Managed-Videos/Public/output`
- `SCAN_INCREMENTAL=true`
- `API_ADDR=:1323`
- `TASK_REFRESH_WINDOW=5m`
...
The scanner only reads `MEDIA_ROOT`. Task creation writes under `OUTPUT`.

## Frontend

```bash
cd web
/opt/homebrew/bin/node ./node_modules/next/dist/bin/next dev --port 3000
```
