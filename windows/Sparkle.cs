...
namespace Sparkle.Windows
{
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (var showEvent = new EventWaitHandle(false, EventResetMode.AutoReset, "Local\\SparkleTranscoderShowLogs", out newEvent))
                using (var mutex = new Mutex(true, "Local\\SparkleTranscoderBackendTray", out newMutex))
                {
                    if (!newMutex)
                    {
                    ...
                    try
                    {
                        string backend = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Sparkle.Backend.exe");
                        using (var app = new TrayApplication(root, backend, showEvent))
                        {
    ...
    internal sealed class TrayApplication : ApplicationContext
    {
        ...
        internal TrayApplication(string repositoryRoot, string executable, EventWaitHandle activationEvent)
        {
            root = repositoryRoot;
            backendPath = executable;
            showEvent = activationEvent;
            if (!File.Exists(Path.Combine(root, "launch-backend.ps1"))) throw new FileNotFoundException("Cannot find launch-backend.ps1 in " + root);
            logs = new LogBuffer(Path.Combine(root, ".sparkle-transcoder", "logs"));
        ...
        internal void StartBackend()
        {
            ...
            try
            {
                ...
                string gateName = "Local\\Sparkle.Start." + token;
                string stopName = "Local\\Sparkle.Stop." + token;
                ...
                // launch-backend.ps1 waits on the gate before starting the server.
                job.Assign(backend);
